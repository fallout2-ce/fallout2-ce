...
void endgamePlaySlideshow()
{
    ...
    for (int index = 0; index < gEndgameEndingsLength; index++) {
        ...
        if (value == ending->value) {
            if (ending->art_num == InterfaceFrameId::INTF_FRM_ID_327) {
...
static void endgameEndingRenderPanningScene(int direction, const char* narratorFileName)
{
    ...
    Art* background = artLock(FrmId(InterfaceFrameId::INTF_FRM_ID_327), &backgroundHandle);
    if (background != nullptr) {
        ...
        endgameEndingLoadPalette(OBJ_TYPE_INTERFACE, static_cast<int>(InterfaceFrameId::INTF_FRM_ID_327));
