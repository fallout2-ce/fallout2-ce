...
void gameMouseRefresh()
{
            ...
            if (gGameMouseMode == GAME_MOUSE_MODE_ARROW) {
                ...
                if (pointedObject != nullptr) {
                    ...
                    if (primaryAction != -1) {
                        if (gameMouseRenderPrimaryAction(mouseX, mouseY, primaryAction, _scr_size.right - _scr_size.left + 1, _scr_size.bottom - _scr_size.top - 99) == 0) {
                            Rect tmp;
                            int fid = buildFid(OBJ_TYPE_INTERFACE, 282, 0, 0);
                            // NOTE: Uninline.
                ...
                if (pointedObject != nullptr) {
                    ...
                    if (gameMouseRenderAccuracy(formattedAccuracy, color) == 0) {
                        Rect tmp;
                        int fid = buildFid(OBJ_TYPE_INTERFACE, 284, 0, 0);
                        // NOTE: Uninline.
                        if (gmouse_3d_set_flat_fid(fid, &tmp) == 0) {
                            tileWindowRefreshRect(&tmp, gElevation);
                        }
    ...
    if (!_gmouse_mapper_mode) {
        int fid = buildFid(OBJ_TYPE_INTERFACE, 0, 0, 0);
    ...
    if ((mouseState & MOUSE_EVENT_LEFT_BUTTON_DOWN_REPEAT) == MOUSE_EVENT_LEFT_BUTTON_DOWN_REPEAT && gGameMouseMode == GAME_MOUSE_MODE_ARROW) {
        ...
        if (targetObj != nullptr) {
            ...
            if (gameMouseRenderActionMenuItems(mouseX, mouseY, actionMenuItems, actionMenuItemsCount, _scr_size.right - _scr_size.left + 1, _scr_size.bottom - _scr_size.top - 99) == 0) {
                Rect cursorRect;
                int fid = buildFid(OBJ_TYPE_INTERFACE, 283, 0, 0);
                // NOTE: Uninline.
...
int gameMouseSetCursor(int cursor)
{
    ...
    int fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseCursorFrmIds[cursor], 0, 0);
    ...
    int width = artGetWidth(mouseCursorFrm, frame, ROTATION_FIRST);
    ...
    unsigned char* mouseCursorFrmData = artGetFrameData(mouseCursorFrm, frame, ROTATION_FIRST);
...
void gameMouseSetMode(int mode)
{
    ...
    fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseModeFrmIds[mode], 0, 0);
...
int gameMouseRenderPrimaryAction(int x, int y, int menuItem, int width, int height)
{
    ...
    int menuItemFid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseActionMenuItemFrmIds[menuItem], 0, 0);
    ...
    int arrowFid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseModeFrmIds[GAME_MOUSE_MODE_ARROW], 0, 0);
    ...
    unsigned char* arrowFrmData = artGetFrameData(arrowFrm, 0, ROTATION_FIRST);
    int arrowFrmWidth = artGetWidth(arrowFrm, 0, ROTATION_FIRST);
    int arrowFrmHeight = artGetHeight(arrowFrm, 0, ROTATION_FIRST);

    unsigned char* menuItemFrmData = artGetFrameData(menuItemFrm, 0, ROTATION_FIRST);
    int menuItemFrmWidth = artGetWidth(menuItemFrm, 0, ROTATION_FIRST);
    ...
    if (maxX < width) {
        ...
        arrowFid = buildFid(OBJ_TYPE_INTERFACE, 285, 0, 0);
        arrowFrm = artLock(arrowFid, &arrowFrmHandle);
        arrowFrmData = artGetFrameData(arrowFrm, 0, ROTATION_FIRST);
...
int gameMouseRenderActionMenuItems(int x, int y, const int* menuItems, int menuItemsLength, int width, int height)
{
    ...
    for (int index = 0; index < menuItemsLength; index++) {
        ...
        int fid = buildFid(OBJ_TYPE_INTERFACE, frmId, 0, 0);
    ...
    int fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseModeFrmIds[GAME_MOUSE_MODE_ARROW], 0, 0);
    CacheEntry* arrowFrmHandle;
    ...
    int arrowWidth = artGetWidth(arrowFrm, 0, ROTATION_FIRST);
    int arrowHeight = artGetHeight(arrowFrm, 0, ROTATION_FIRST);

    int menuItemWidth = artGetWidth(menuItemFrms[0], 0, ROTATION_FIRST);
    ...
    if (x + arrowWidth + menuItemWidth - 1 < width) {
        arrowData = artGetFrameData(arrowFrm, 0, ROTATION_FIRST);
        ...
        artUnlock(arrowFrmHandle);
        fid = buildFid(OBJ_TYPE_INTERFACE, 285, 0, 0);
    ...
    for (int index = 0; index < menuItemsLength; index++) {
        unsigned char* data = artGetFrameData(menuItemFrms[index], 0, ROTATION_FIRST);
...
int gameMouseHighlightActionMenuItemAtIndex(int menuItemIndex)
{
    ...
    CacheEntry* handle;
    int fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseActionMenuItemFrmIds[gGameMouseActionMenuItems[gGameMouseActionMenuHighlightedItemIndex]], 0, 0);
    ...
    fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseActionMenuItemFrmIds[gGameMouseActionMenuItems[menuItemIndex]] - 1, 0, 0);
    art = artLock(fid, &handle);
    if (art == nullptr) {
        return -1;
    }

    data = artGetFrameData(art, 0, ROTATION_FIRST);
...
int gameMouseRenderAccuracy(const char* string, int color)
{
    ...
    int fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseModeFrmIds[GAME_MOUSE_MODE_CROSSHAIR], 0, 0);
    ...
    int crosshairFrmWidth = artGetWidth(crosshairFrm, 0, ROTATION_FIRST);
    int crosshairFrmHeight = artGetHeight(crosshairFrm, 0, ROTATION_FIRST);
    unsigned char* crosshairFrmData = artGetFrameData(crosshairFrm, 0, ROTATION_FIRST);
...
int gameMouseRenderActionPoints(const char* string, int color)
{
    ...
    int fid = buildFid(OBJ_TYPE_INTERFACE, 1, 0, 0);
    gameMouseSetBouncingCursorFid(fid);
...
int gameMouseObjectsInit()
{
    ...
    fid = buildFid(OBJ_TYPE_INTERFACE, 0, 0, 0);
    if (objectCreateWithFidPid(&gGameMouseBouncingCursor, fid, -1) != 0) {
        return -1;
    }

    fid = buildFid(OBJ_TYPE_INTERFACE, 1, 0, 0);
...
int gameMouseActionMenuInit()
{
    ...
    fid = buildFid(OBJ_TYPE_INTERFACE, 283, 0, 0);
    ...
    fid = buildFid(OBJ_TYPE_INTERFACE, 282, 0, 0);
    ...
    fid = buildFid(OBJ_TYPE_INTERFACE, 284, 0, 0);
    ...
    gGameMouseActionMenuFrmWidth = artGetWidth(gGameMouseActionMenuFrm, 0, ROTATION_FIRST);
    ...
    gGameMouseActionMenuFrmData = artGetFrameData(gGameMouseActionMenuFrm, 0, ROTATION_FIRST);

    gGameMouseActionPickFrmWidth = artGetWidth(gGameMouseActionPickFrm, 0, ROTATION_FIRST);
    ...
    gGameMouseActionPickFrmData = artGetFrameData(gGameMouseActionPickFrm, 0, ROTATION_FIRST);

    gGameMouseActionHitFrmWidth = artGetWidth(gGameMouseActionHitFrm, 0, ROTATION_FIRST);
    ...
    gGameMouseActionHitFrmData = artGetFrameData(gGameMouseActionHitFrm, 0, ROTATION_FIRST);

    gGameMouseBouncingCursorFrmWidth = artGetWidth(gGameMouseBouncingCursorFrm, 0, ROTATION_FIRST);
    ...
    gGameMouseBouncingCursorFrmData = artGetFrameData(gGameMouseBouncingCursorFrm, 0, ROTATION_FIRST);

    gGameMouseHexCursorFrmWidth = artGetWidth(gGameMouseHexCursorFrm, 0, ROTATION_FIRST);
    ...
    gGameMouseHexCursorFrmData = artGetFrameData(gGameMouseHexCursorFrm, 0, ROTATION_FIRST);
...
{
    int fid = buildFid(OBJ_TYPE_INTERFACE, gGameMouseModeFrmIds[gGameMouseMode], 0, 0);
