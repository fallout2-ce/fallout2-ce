...
static int db_buffer_remaining(DB_FILE* stream);
            ...
            case 16:
                if (stream->field_10 != 0) {
                    ...
                    if (stream->field_10 != 0 && (stream->flags & 0x2) != 0 && ch == '\r') {
                        if (db_buffer_remaining(stream) <= 0) {
                            break;
                        }
    ...
    if (stream != NULL) {
        if ((stream->flags & 0x4) != 0) {
            ...
            case 64:
                ...
                if (v1 >= stream->field_1C && v1 < stream->field_1C + stream->field_24) {
                    stream->field_20 = v1;
                    stream->field_10 = stream->field_C - offset;
                    rc = 0;
                } else {
                    if (offset < current_offset) {
                        ...
                        stream->field_10 -= db_buffer_remaining(stream);
                        stream->field_20 = stream->field_1C + stream->field_24;
                        db_preload_buffer(stream);
        ...
        if (db_find_empty_position(&pos) == 0) {
            ...
            if ((flags & 0x4) != 0) {
                ...
                case 64:
                    ...
                    current_database->files[pos].field_24 = 0;
                    ptr = &(current_database->files[pos]);
                    break;
                }
            }
...
static int db_find_dir_entry(char* path, dir_entry* de)
{
    ...
    for (ch = normalized_path; *ch != '\0'; ch++) {
        if (*ch == '/') {
            *ch = '\\';
        }
    }
...
static void db_preload_buffer(DB_FILE* stream)
{
    ...
    if ((stream->flags & 0x8) != 0 && (stream->flags & 0xF0) == 64) {
        if (stream->field_10 != 0) {
            if (stream->field_20 >= stream->field_1C + stream->field_24) {
                if (fseek(stream->database->stream, stream->field_18, SEEK_SET) == 0) {
                    if (fread_short(stream->database->stream, &v1) == 0) {
                        bytesRead = 0;
                        if ((v1 & 0x8000) != 0) {
                            v1 &= ~0x8000;
                            bytesRead = fread(stream->field_1C, 1, v1, stream->database->stream);
                        } else {
                            bytesRead = lzss_decode_to_buf(stream->database->stream, stream->field_1C, v1);
                        }

                        stream->field_20 = stream->field_1C;
                        stream->field_24 = bytesRead;
                        stream->field_18 = ftell(stream->database->stream);
                    }
                }
            }
...
static int db_buffer_remaining(DB_FILE* stream)
{
    int remaining = stream->field_24 - (stream->field_20 - stream->field_1C);
