...
int map_scr_remove_spatial(int tile, int elevation)
{
    ...
    while (obj != NULL) {
        if (obj->tile == tile && FrmId(InterfaceFrameId::INTF_FRM_ID_3).fid() == obj->fid) {
...
int map_scr_remove_all_spatials()
{
    ...
    for (elevation = 0; elevation < ELEVATION_COUNT; elevation++) {
        ...
        obj = objectFindFirstAtElevation(elevation);
        while (obj != NULL) {
            if (FrmId(InterfaceFrameId::INTF_FRM_ID_3).fid() == obj->fid) {
...
{
    constexpr FrmId kMarkerFrmId = FrmId(InterfaceFrameId::INTF_FRM_ID_3);
