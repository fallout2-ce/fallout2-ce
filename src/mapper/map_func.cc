...
void placeTile(int pid, int fid)
{
    ...
    if (tileRoofIsVisible()) {
        int oldRoofFid = FrmId(tileFrameIdFromFid(oldValue >> 16));
        if (oldRoofFid == fid) {
            return;
        }
        ...
        *squarePtr = (newRoofWord << 16) | (oldValue & 0xFFFF);
        ...
        int oldFloorFid = FrmId(tileFrameIdFromFid(oldValue));
        if (oldFloorFid == fid) {
            return;
        }
...
void copyTile()
{
    ...
    int blankFid = FrmId(TILE_FRM_ID_1);
            ...
            if ((mouseEvent & MOUSE_EVENT_LEFT_BUTTON_DOWN) != 0) {
                if (!_mouse_click_in(0, 0,
                    ...
                    if (hit != nullptr) {
                        ...
                        int exitGridFid = FrmId(INTF_FRM_ID_3);
