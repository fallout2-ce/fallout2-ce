...
static void op_get_critter_current_ap(Program* program)
{
    ...
    int actionPoints = 0;
    if (critter != nullptr && FrmId(critter).objectType() == OBJ_TYPE_CRITTER) {
...
static void op_get_tile_fid(Program* program)
{
    ...
    int squareData = _square[elevation]->tileFid[squareTile];
    ...
    case 1:
        // roof tile frame id
        programStackPushInteger(program, TileFrmId(roofTileFidFromCombinedTileFid(squareData)).frameId().id);
        break;
    case 2:
    ...
    default:
        // floor tile frame id
        programStackPushInteger(program, TileFrmId(floorTileFidFromCombinedTileFid(squareData)).frameId().id);
        break;
    }
}
