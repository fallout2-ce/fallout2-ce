...
int proto_item_init(Proto* proto, int pid)
{
    ...
    proto->item.fid = buildFid(protoNum - 1);
    if (!artExists(proto->item.fid)) {
        proto->item.fid = buildFid(ITEM_FRM_ID_FIRST);
    }
...
int proto_wall_init(Proto* proto, int pid)
{
    ...
    proto->wall.fid = buildFid(num - 1);
    if (!artExists(proto->wall.fid)) {
        proto->wall.fid = buildFid(WALL_FRM_ID_FIRST);
    }
...
int proto_tile_init(Proto* proto, int pid)
{
    ...
    proto->tile.fid = buildFid(num - 1);
    if (!artExists(proto->tile.fid)) {
        proto->tile.fid = buildFid(TILE_FRM_ID_FIRST);
    }
...
int proto_misc_init(Proto* proto, int pid)
{
    ...
    proto->misc.fid = buildFid(num - 1);
    if (!artExists(proto->misc.fid)) {
        proto->misc.fid = buildFid(MISC_FRM_ID_FIRST);
    }
