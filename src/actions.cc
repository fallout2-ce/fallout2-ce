...
void showDamage(Attack* attack, AnimationType attackerAnimation, int delay)
{
    for (int index = 0; index < attack->extrasLength; index++) {
        ...
        if (FrmId(object->fid).objectType() == OBJ_TYPE_CRITTER) {
...
int actionUseSkill(Object* user, Object* target, Skill skill)
{
    ...
    AnimationType anim = (FrmId(target->fid).objectType() == OBJ_TYPE_CRITTER && critterIsProne(target)) ? ANIM_MAGIC_HANDS_GROUND : ANIM_MAGIC_HANDS_MIDDLE;
...
int actionExplode(int tile, int elevation, int minDamage, int maxDamage, Object* sourceObj, bool animate)
{
    ...
    if (critter != nullptr) {
        if (FrmId(critter->fid).objectType() != OBJ_TYPE_CRITTER || (critter->data.critter.combat.results & DAM_DEAD) != DAM_NONE) {
            critter = nullptr;
        }
    }
