...
| Virtual file system | fs_create<br>fs_copy<br>fs_find<br>fs_read/write_*<br>fs_delete<br>fs_size<br>fs_pos<br>fs_seek<br>fs_resize | 🚫 | Not implemented. Open an issue if you have a use case for these. |
...
| Art and appearance | art_exists<br>refresh_pc_art<br>art_cache_clear<br>set_hero_race<br>set_hero_style | implemented: art_exists, refresh_pc_art, art_cache_clear, set_hero_race, set_hero_style | `[appearance] hero_appearance` or `[qol] enable_hero_appearance_mod` enables CE-native Hero Appearance support. `set_hero_race` and `set_hero_style` update CE's race/style state, refresh mounted appearance packages, and recompute selected PC art through CE's art/FID resolution. Selectable player appearances can be backed by alternate critter art packages or registry entries for complete critter animation sets. CE does not emulate sfall temporary FRM file patching or fs_* virtual file writes. |
...
| Hero Appearance | set_dm/df_model<br>hero_select_win | implemented: set_dm_model, set_df_model, hero_select_win | Model opcodes safely update known male/female dude model slots used by `refresh_pc_art`. `hero_select_win` opens a native race/style selector when Hero Appearance is enabled; it commits on Done/Enter, cancels on Esc, reads CE-native appearance registry entries, includes available `appearance\h{m|f}r##s##` packages for the current character gender, and previews highlighted critter art. This is CE-native package/path mounting and art/FID resolution, not full sfall Hero Appearance compatibility. The sfall character-screen tab integration is not cloned. |
