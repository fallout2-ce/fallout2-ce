...
| Art and appearance | art_exists<br>refresh_pc_art<br>art_cache_clear<br>set_hero_race<br>set_hero_style | implemented: art_exists, refresh_pc_art, art_cache_clear, set_hero_race, set_hero_style | `[appearance] hero_appearance` enables CE-native Hero Appearance support and migrates from sfall `EnableHeroAppearanceMod`. `set_hero_race` and `set_hero_style` update CE's race/style state, refresh mounted appearance packages, and recompute selected PC art through CE's art lookup. CE does not emulate sfall temporary FRM file patching or fs_* virtual file writes. |
...
| Hero Appearance | set_dm/df_model<br>hero_select_win | implemented: set_dm_model, set_df_model, hero_select_win | Model opcodes update the default male/female dude model slots used by `refresh_pc_art`. `hero_select_win` opens a native race/style selector when `[appearance] hero_appearance` is enabled; it commits on Done/Enter, cancels on Esc, checks available `appearance\h{m|f}r##s##` packages for the current character gender, and previews highlighted critter art. This is CE-native package mounting/art remapping, not full sfall Hero Appearance compatibility. The sfall character-screen tab integration is not cloned. |
