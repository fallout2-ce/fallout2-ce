...
| Hero Appearance | set_dm/df_model<br>hero_select_win | implemented: set_dm_model, set_df_model, hero_select_win | Model opcodes safely update known male/female dude model slots used by `refresh_pc_art`. `hero_select_win` opens the CE-native Hero Race / Hero Style picker and applies selections through CE's race/style state, package/path mounting, and art/FID resolution. This is not full sfall virtual-file compatibility. |
