...
| Perks and traits | get/set_perk_owed<br>set_perk_freq<br>set_perk_image<br>set_perk_*<br>set_pyromaniac_mod<br>apply_heaveho_fix<br>set_swiftlearner_mod<br>has/set_fake_perk<br>has/set_fake_trait<br>set_selectable_perk<br>set_perkbox_title<br>show/hide_real_perks<br>perk_add_mode<br>clear_selectable_perks<br>add/remove_trait<br>seq_perk_freq | implemented: get/set_perk_owed, set_perk_freq, set_perk_image, set_perk_* | `ddraw.ini` `PerksFile` supports sfall `Perks.ini` tweaks and existing perk overrides. New perks with IDs 119..255 are not yet supported. |
| Virtual file system | fs_create<br>fs_copy<br>fs_find<br>fs_read/write_*<br>fs_delete<br>fs_size<br>fs_pos<br>fs_seek<br>fs_resize | implemented: fs_copy, fs_find, fs_delete | Only supports `fs_copy` aliases for asset lookup. `sound\sfx\` redirects are excluded because SFX metadata is cached before scripts run. Virtual file contents, reads/writes, resize, and savegame persistence are not implemented. |
...
| Global variables | set_sfall_global<br>get_sfall_global_int<br>get_sfall_global_float | ✅ | - |
