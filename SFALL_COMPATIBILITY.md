...
| Hero Appearance | set_dm/df_model<br>hero_select_win | implemented: set_dm_model, set_df_model<br>deferred: hero_select_win | Model opcodes safely update known male/female dude model slots used by `refresh_pc_art`. The modal appearance picker is intentionally not exposed as `hero_select_win`; the CE-native picker currently lives in character creation, while direct runtime switching remains script-driven through `set_hero_race`, `set_hero_style`, and `refresh_pc_art`. This is CE-native package/path mounting and art/FID resolution, not full sfall Hero Appearance compatibility. |
